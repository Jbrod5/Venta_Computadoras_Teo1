...
<br>
    <h2>Editar Ensamble</h2>
    <br>

    <form id="ensambleForm" action="{{ route('ensambles.update', $ensamble->id_ensamble) }}" method="POST">
    @csrf
    @method('PUT')
        ...
        @php
            ...
            // IDs de los componentes que ya tiene este ensamble
            $componentesSeleccionados = $ensamble->componentes->pluck('id_componente')->toArray();
        @endphp
                    ...
                    <div class="card p-2 h-100 componente-card" onclick="toggleCheckbox({{ $componente->id_componente }})">
                        <div class="form-check">
                            <input class="form-check-input me-2" type="checkbox" 
                                   ...
                                   {{ in_array($componente->id_componente, $componentesSeleccionados) ? 'checked' : '' }}>
        ...
        <div class="mt-4">
            <button type="submit" class="btn btn-primary">Actualizar Ensamble</button>
            <a href="{{ route('ensambles.show', $ensamble->id_ensamble) }}" class="btn btn-secondary">Cancelar</a>
        </div>
    </form>
    <br><br><br><br>
